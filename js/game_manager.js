function GameManager(){
    ...
    var grid = new Grid(22, 10); // row, col
    var rpg = new RandomPieceGenerator();
    var ai = new AI({ // weight compute by GA
    ...
    pauseButton.onclick = function(){
        if (gravityTimer.status()){
            if (!isAiActive){
                pauseButton.style.backgroundColor = "#e9e9ff";
                gravityTimer.stop();
                cancelWorkingPieceDropAnimation();
                isKeyEnabled = true;
            }
            ...
            if (!isAiActive){
                pauseButton.style.backgroundColor = "#f9f9f9";
                gravityTimer.start();
            }
        }
    }
